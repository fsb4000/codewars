...
Describe "maxTriSum" {
  ...
  It "should pass random tests" {
    ...
    function modelSolution ([int[]]$numbers)
    {
    ...
    for ([int]$i = 0; $i -lt 100;++ $i) {
      [int]$randomSize = $rnd.Next(3,202);
      [int[]]$randomInput = @(0) * $randomSize;
      for ([int]$j = 0; $j -lt $randomInput.Length;++ $j) {
        $randomInput[$j] = $rnd.Next(-100,101);
      }
      [int]$expected = maxTriSum ($randomInput);
      [int]$actual = modelSolution ($randomInput);
